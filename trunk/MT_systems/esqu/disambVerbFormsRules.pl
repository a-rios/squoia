...
my $num_args = $#ARGV + 1;
if ($num_args > 1) {
  print STDERR "\nUsage:  perl disambVerbFormsRules.pl (-EVID)\n";
  	print STDERR "\nArgument evidentiality (EVID) is optional,\n
  					only possible value is -indirect: all ambiguous past forms will be translated with -sqa\n
  					none given, direct evidentiality (default): all ambiguous past forms will be translated with -rqa\n";		
  exit;
}
...
$evid =~ s/^\-//;
# if evidentiality is set
if ($evid ne ''){
	if ($evid ne 'indirect'){
		print STDERR "wrong value for argument evidentiality: $evid\nonly possible value is 'indirect' (no argument = direct = default)\n";
		print STDERR "will treat as 'direct' (-rqa)!\n";
		$evid = 'direct';
	}
}
#no argument: default = direct
else{
	$evid='direct';
}
 			...
 			{ 
				...
				elsif($conjunction && $conjunction->getAttribute('lem') =~ /mientras|al_tiempo_que/ )
				{
 				...
 				elsif($verbChunk->getAttribute('si') eq 'suj') 
 				{
 					...
 					if($headVerbCHunk)
 					{
 						my $headlem = &getMainVerb($headVerbCHunk);
