...
#	}
#	else{
#		my $encoding_name = Encode::Detect::Detector::detect($n->getAttribute('form'));
...
#}
	...
	#		}
			...
			{
				my $headNoun = squoia::util::getHeadNoun($relClause);
				...
				unless($headNoun == -1)
				{
					...
					if($finiteVerb)
					{
						...
						{ 
							...
							if(!squoia::util::isCongruentHeadRelClause($headNoun, $finiteVerb) && $headNounLem !~ /_/)
							{
								&setVerbform($relClause,0);
							}
							...
							{
							#get verb lemma
							my $mainVerb = squoia::util::getMainVerb($relClause);
							if($mainVerb)
							{
...
#      </CHUNK>
	...
	if($verbchunk && $isSubj != 2)
	{
		...
		foreach my $verbform (@verbforms)
		{
			...
			if($isSubj == 1)
			{
				my $headnoun = squoia::util::getHeadNoun($verbchunk);
				my $verbMI = $verbform->getAttribute('mi');
				unless($headnoun== -1)
				{
					if($verbMI =~ /1|2|3/ && $headnoun && squoia::util::isCongruentHeadRelClause($headnoun, $verbform))
					{
						$verbform->setAttribute('verbform', 'rel:agentive');
					}
					...
					else
					{
						...
						if($finiteVerb && $headnoun &&  $finiteVerb->getAttribute('mi') =~ /1|2|3/ && squoia::util::isCongruentHeadRelClause($headnoun, $finiteVerb))
						{
							$verbform->setAttribute('verbform', 'rel:agentive');
						}	
					}
