...
\usepackage{hyperref}
 \hypersetup{colorlinks, urlcolor= blue!50!black, linkcolor=blue!50!black, citecolor=green!40!black}
...
\begin{tikzpicture}
  ...
  \draw (8,-2) node (-nman) {-nman};
    ...
    \path[-] (-chi) edge  node[label=left:$mod$] {} (panpaya);
...
\begin{tikzpicture}
      \draw (-4,0.5) node (1) {ejemplo \ref{Ex:coloc}:};
      \draw (0,0) node (pukllanku) {pukllanku};
  \draw (-2,-1) node (llank'anku) {llank'anku};
      \draw (1,-1) node (ima) {ima};
  \draw (-2,-2.5) node (kaypi) {\textcolor{blue}{kaypi}};
      \path[-] (llank'anku) edge  node[label=left:$co$]{} (pukllanku);
      \path[-] (kaypi) edge  node[label=left:\textcolor{blue}{$loc$}]  {} (llank'anku);
      \path[-] (ima) edge  node[label=right:$linker$]  {} (pukllanku);
% \draw[color=blue,dashed] (kaypi) .. controls (-0.7,-2)
%  and (-0.5,-1.5) .. (pukllanku);
  \path[-,dashed,color=blue] (kaypi) edge [out=30,in=-90] node[label=below:\textcolor{blue}{$loc$}] {} (pukllanku);
...
\includegraphics[width=7cm]{tred51.png}
}}
\end{center}
