...
import sys, re, random, colorsys, pdb
...
all_dok=open(sys.argv[1],'r').read()
doks = re.split('#end document.*',all_dok)
for dok in doks:
    lines=dok.split('\n')
    ids=[]
    coref={}    # colors dict
    ...
    for line in lines:
        if not line=='':
            if not line.strip().endswith('_') and not line.strip().endswith('-') and not line.startswith('#'):
                l=line.split('\t')
                ms=l[-1].split('|')
                for m in ms:
